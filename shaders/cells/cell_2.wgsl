// Phagocyte Shader - Cell Type 2
// Darker appearance with membrane ripples for engulfing cells
...
// Simple noise function for membrane ripples
...
fn fs_main(in: VertexOutput) -> FragmentOutput {
    ...
    // Add membrane ripples using noise
    let ripple_scale = 10.0;
    let ripple = simple_noise(in.uv * ripple_scale + camera.time * 0.3);
    let ripple_strength = 0.1;
    let perturbed_normal = billboard_normal + vec3<f32>(ripple * ripple_strength, ripple * ripple_strength, 0.0);

    let world_normal = normalize(
        in.cam_right * perturbed_normal.x +
        in.cam_up * perturbed_normal.y +
        in.to_camera * perturbed_normal.z
    );
    ...
    // Darker lighting for Phagocytes
    ...
    // Apply darker color with red/purple tint for Phagocytes
    let phagocyte_tint = vec3<f32>(1.0, 0.6, 0.8);  // Red-purple tint
    let tinted_color = in.color.rgb * phagocyte_tint * 0.7;  // Darker with tint

    let diffuse = ndotl * lighting.light_color;
    let lit_color = tinted_color * (lighting.ambient + diffuse) + specular + fresnel_color;
    let final_color = lit_color + tinted_color * in.visual_params.w;
