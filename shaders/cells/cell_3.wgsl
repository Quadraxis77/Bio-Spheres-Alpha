// Photocyte Shader - Cell Type 3
// Bright green/yellow appearance with glowing effect for photosynthetic cells
...
fn fs_main(in: VertexOutput) -> FragmentOutput {
    ...
    // Brighter lighting for Photocytes
    ...
    let specular = spec * in.visual_params.x * lighting.light_color * 1.5;  // Enhanced specular

    let fresnel = pow(1.0 - max(dot(world_normal, view_dir), 0.0), 3.0);
    let fresnel_color = fresnel * in.visual_params.z * 1.3;  // Enhanced fresnel

    // Apply bright green/yellow tint for Photocytes
    let photosynthetic_tint = vec3<f32>(0.8, 1.0, 0.4);  // Yellow-green tint
    let tinted_color = in.color.rgb * photosynthetic_tint * 1.2;  // Brighten by 20%

    let diffuse = ndotl * lighting.light_color;
    let lit_color = tinted_color * (lighting.ambient + diffuse) + specular + fresnel_color;

    // Add pulsing glow effect
    let glow_pulse = 0.1 + 0.05 * sin(camera.time * 2.0);
    let glow = tinted_color * glow_pulse;

    let final_color = lit_color + tinted_color * in.visual_params.w + glow;
