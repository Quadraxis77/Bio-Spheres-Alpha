...
fn anti_gravity_direction(pos: vec3<f32>) -> vec3<f32> {
    if (camera.gravity_mode == 3u) {
        let len_pos = length(pos);
        if (len_pos > 0.001) {
            return normalize(pos) * select(1.0, -1.0, camera.gravity < 0.0);
        }
        return vec3<f32>(0.0, 1.0, 0.0);
    }

    let sign = select(1.0, -1.0, camera.gravity < 0.0);
    if (camera.gravity_mode == 0u) {
        return vec3<f32>(sign, 0.0, 0.0);
    }
    if (camera.gravity_mode == 2u) {
...
fn vs_main(in: VertexInput, @builtin(instance_index) instance_index: u32) -> VertexOutput {
    ...
    let feather_length = clamp(instance.type_data_0.x, 0.2, 2.8);
    let feather_width = clamp(instance.type_data_0.y, 0.025, 0.22);
    ...
    let width = feather_width * instance.radius * mix(0.55, 0.22, t) * select(1.0, 0.72, line_kind > 0.5);
    let normal_offset = normalize(cross(tangent, equator_normal)) * ribbon_side * width;

    let local_position = p * instance.radius + normal_offset;
    let world_position = instance.position + quat_rotate(instance.rotation, local_position);

    let world_normal = normalize(quat_rotate(instance.rotation, equator_normal));
    let ndotl = max(dot(world_normal, -normalize(lighting.light_dir)), 0.0);

    out.clip_position = camera.view_proj * vec4<f32>(world_position, 1.0);
    out.color = vec4<f32>(mix(instance.color.rgb * 0.78, vec3<f32>(0.62, 0.95, 0.82), 0.35), instance.color.a);
    out.feather_t = t;
