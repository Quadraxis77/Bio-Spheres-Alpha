// Volumetric Fog Composite Shader
// Upscales the half-resolution fog texture back to full resolution.
// Uses a 9-tap weighted blur to hide ray-march grain before blending.
// blur_radius controls the kernel spread: 0 = nearest, 1.5 = moderate, 3+ = very soft.

struct CompositeParams {
    blur_radius: f32,
    _pad0: f32,
    _pad1: f32,
    _pad2: f32,
}

@group(0) @binding(0) var fog_tex: texture_2d<f32>;
@group(0) @binding(1) var fog_samp: sampler;
@group(0) @binding(2) var<uniform> cp: CompositeParams;

struct VertexOutput {
    @builtin(position) position: vec4<f32>,
    @location(0) uv: vec2<f32>,
}

@vertex
fn vs_main(@builtin(vertex_index) vertex_index: u32) -> VertexOutput {
    var out: VertexOutput;
    let x = f32(i32(vertex_index & 1u) * 4 - 1);
    let y = f32(i32(vertex_index >> 1u) * 4 - 1);
    out.position = vec4<f32>(x, y, 0.0, 1.0);
    out.uv = vec2<f32>(x * 0.5 + 0.5, 1.0 - (y * 0.5 + 0.5));
    return out;
}

@fragment
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    let d   = cp.blur_radius / vec2<f32>(textureDimensions(fog_tex, 0));
    let uv  = in.uv;

    // 9-tap weighted tent kernel.  blur_radius scales all offsets uniformly so
    // the user can dial from tight (0.5) to very soft (3+).
    var col = vec4<f32>(0.0);
    let h = d * 0.5;
    col += textureSample(fog_tex, fog_samp, uv + vec2<f32>(-h.x, -h.y)) * 0.0625;
    col += textureSample(fog_tex, fog_samp, uv + vec2<f32>( h.x, -h.y)) * 0.0625;
    col += textureSample(fog_tex, fog_samp, uv + vec2<f32>(-h.x,  h.y)) * 0.0625;
    col += textureSample(fog_tex, fog_samp, uv + vec2<f32>( h.x,  h.y)) * 0.0625;
    col += textureSample(fog_tex, fog_samp, uv + vec2<f32>(-d.x,  0.0)) * 0.125;
    col += textureSample(fog_tex, fog_samp, uv + vec2<f32>( d.x,  0.0)) * 0.125;
    col += textureSample(fog_tex, fog_samp, uv + vec2<f32>( 0.0, -d.y)) * 0.125;
    col += textureSample(fog_tex, fog_samp, uv + vec2<f32>( 0.0,  d.y)) * 0.125;
    col += textureSample(fog_tex, fog_samp, uv)                          * 0.5;

    return col;
}
