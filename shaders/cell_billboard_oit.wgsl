...
fn fs_main(in: VertexOutput) -> FragmentOutput {
    ...
    let linear_depth = sphere_clip.w;
    
    // Membrane noise - transform world normal by inverse cell rotation to get local-space
    // This makes the noise texture rotate with the cell
    let local_noise_normal = quat_rotate_inverse(in.cell_rotation, world_normal);
    let local_surface_pos = local_noise_normal;  // Unit sphere in local space
    ...
    if (is_nucleus) {
        ...
        let nuc_local_noise_normal = quat_rotate_inverse(in.cell_rotation, nuc_world_normal);
        let nuc_local_surface = nuc_local_noise_normal * nucleus_size + vec3<f32>(50.0);
        let nuc_perturbed_normal = perturb_normal(nuc_world_normal, nuc_local_surface, noise_scale * 1.5, noise_strength * 0.7, anim_time * 0.5);
    ...
    // Composite - cytoplasm and nucleus are always opaque internally
    ...
    // Cell itself is always fully opaque for OIT purposes
    // The membrane opacity only affects internal compositing, not overall cell transparency
    let alpha = 1.0;
    let weight = calculate_weight(linear_depth / 100.0, alpha);
