...
pub trait TabViewer {
    ...
    /// Returns `true` if this tab is a placeholder panel.
    ///
    /// Placeholder panels are special panels that hold space in the layout but don't
    /// display tab buttons and can't be dragged. They're useful for creating fixed
    /// layout regions that contain other tabs.
    /// By default, `false` is always returned.
    fn is_placeholder(&self, _tab: &Self::Tab) -> bool {
        false
    }

    /// Returns `true` if the tab should be draggable.
    ///
    /// By default, `true` is always returned unless the tab is a placeholder or viewport.
    fn is_draggable(&self, tab: &Self::Tab) -> bool {
        !self.is_placeholder(tab) && !self.is_viewport(tab)
    }

    /// Returns `true` if the tab button should be hidden.
    ///
    /// By default, tab buttons are hidden for placeholder panels and viewports.
    fn hide_tab_button(&self, tab: &Self::Tab) -> bool {
        self.is_placeholder(tab) || self.is_viewport(tab)
    }

    /// Returns `true` if this tab should be treated as a viewport.
    ///
    /// Viewport tabs receive special rendering treatment: their tab buttons are hidden,
    /// they can't be dragged, and their background is transparent to show 3D content.
    /// By default, `false` is always returned.
    fn is_viewport(&self, _tab: &Self::Tab) -> bool {
        false
    }

    /// Returns the minimum fraction (0.0 to 1.0) that this tab's panel can be resized to.
    ///
    /// This is useful for preventing panels from becoming too small.
    /// By default, returns `None` (no minimum enforced).
    fn min_fraction(&self, _tab: &Self::Tab) -> Option<f32> {
        None
    }
}
