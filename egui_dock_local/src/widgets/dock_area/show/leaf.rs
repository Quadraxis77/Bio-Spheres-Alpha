...
impl<Tab> DockArea<'_, Tab> {
    ...
    fn tab_bar(
        ...
        // Check if all tabs in this leaf should have their buttons hidden
        let all_tabs_hidden = {
            let leaf = self.dock_state[surface_index][node_index]
                .get_leaf_mut()
                .expect("This node must be a leaf");
            leaf.tabs.iter().all(|tab| tab_viewer.hide_tab_button(tab))
        };
    ...
    fn tab_title(
        ...
        if ui.ctx().dragged_id().is_none() && self.draggable_tabs && is_draggable {
