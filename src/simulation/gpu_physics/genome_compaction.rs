...
impl GenomeCompactionSystem {
    ...
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue, max_genomes: usize) -> Self {
        ...
        let shader_module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(
                include_str!("../../../shaders/genome_compaction.wgsl").into(),
            ),
        });
        ...
        queue.write_buffer(
            &max_genomes_buffer,
            0,
            bytemuck::bytes_of(&max_genomes_value),
        );
    ...
    fn upload_genome_data(&self, queue: &wgpu::Queue, genome_manager: &GenomeBufferManager) {
        ...
        if !reference_counts.is_empty() {
            queue.write_buffer(
                &self.reference_count_buffer,
                0,
                bytemuck::cast_slice(&reference_counts),
            );
        }
        if !marked_for_deletion.is_empty() {
            queue.write_buffer(
                &self.marked_for_deletion_buffer,
                0,
                bytemuck::cast_slice(&marked_for_deletion),
            );
        }
    }
