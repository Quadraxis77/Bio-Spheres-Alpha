...
use super::{AdhesionBuffers, GpuCellDataExtraction, GpuTripleBufferSystem, InspectedCellData};
...
impl ReadbackResult {
    ...
    pub fn debug_print_organism_id(&self) {
        println!(
            "[DEBUG] Cell Inspector Readback - Cell {}: Organism ID = {} (raw: 0x{:08X})",
        ...
        if self.data.organism_id == u32::MAX {
            ...
            println!(
                "[DEBUG]   -> Organism member, root cell index = {}",
                self.data.organism_id
            );
        }
    }
}
            ...
            if let Some(readback_id) = self.current_readback_id {
                if let Some(data_bytes) = readback_manager.get_result_bytes(readback_id) {
                    ...
                    if data_bytes.len() == std::mem::size_of::<InspectedCellData>() {
                        ...
                        eprintln!(
                            "Readback data size mismatch: expected {}, got {}",
                    ...
                    // Readback failed
                    eprintln!(
                        "Cell data extraction failed for readback ID {:?}",
                        readback_id
                    );
                    self.current_readback_id = None;
...
impl AsyncReadbackManager {
    ...
    pub fn initiate_readback(
        ...
        encoder.copy_buffer_to_buffer(source_buffer, 0, &_staging_buffer, 0, size);
