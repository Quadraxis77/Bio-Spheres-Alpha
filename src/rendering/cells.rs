        ...
        for (cell_index, cell_type_index) in cells_with_types {
            ...
            let instance = CellInstance {
                ...
                visual_params: [
                    ...
                    {
                        let base = if mode_settings.emissive < 0.001
                            && cell_type == CellType::Luminocyte
                        {
                            0.5
                        } else {
                        ...
                        if cell_type == CellType::Luminocyte {
                            let nutrients =
                                state.nutrients.get(cell_index).copied().unwrap_or(100.0);
                            let nutrient_factor = ((nutrients - 1.0) / 9.0).clamp(0.0, 1.0);
                            let smoothed =
                                nutrient_factor * nutrient_factor * (3.0 - 2.0 * nutrient_factor);
                            base * smoothed
                        } else {
                            base
                        }
        ...
        // that otherwise disables GPU early-z entirely.
        {
            let mut depth_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                ...
                depth_stencil_attachment: Some(wgpu::RenderPassDepthStencilAttachment {
                    view: &self.depth_view,
                    depth_ops: Some(wgpu::Operations {
                        load: wgpu::LoadOp::Load, // Scene clear pass already initialized depth to 1.0; Load preserves cave/boulder depths written before cells
                        store: wgpu::StoreOp::Store,
                    }),
                    stencil_ops: None,
                }),
