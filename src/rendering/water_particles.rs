//! Water particle system renderer (GPU-based)
//!
//! Renders water (fluid type 1) as small, prominent billboarded quads using GPU compute
//! to extract water positions directly from the fluid state buffer.
//! This makes individual water particles more visible without affecting the overall water mesh.
...
/// GPU-based water particle system renderer
pub struct WaterParticleRenderer {
    // Render pipeline
    render_pipeline: wgpu::RenderPipeline,

    // Compute pipeline for extracting water positions
...
impl WaterParticleRenderer {
    pub fn new(
        ...
        let render_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Water Particle Render Shader"),
            source: wgpu::ShaderSource::Wgsl(
                include_str!("../../shaders/water_particles.wgsl").into(),
            ),
        });

        // Create compute shader for extraction
        let extract_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Water Extract Compute Shader"),
            source: wgpu::ShaderSource::Wgsl(
                include_str!("../../shaders/water_extract.wgsl").into(),
            ),
        });

        // Create particle buffer
        let particle_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Water Particle Buffer"),
            size: (std::mem::size_of::<WaterParticle>() * max_particles as usize) as u64,
        ...
        let params_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Water Extract Params"),
        ...
        let vertex_buffer_layout = wgpu::VertexBufferLayout {
            array_stride: std::mem::size_of::<WaterParticle>() as u64,
        ...
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Water Particle Pipeline"),
    ...
    /// Set how prominent water particles should be (0.0 = barely visible, 1.0 = very prominent)
    pub fn set_prominence_factor(&mut self, factor: f32) {
        self.prominence_factor = factor.clamp(0.0, 1.0);
    }
    ...
    /// Get the particle buffer (for indirect draw if needed)
