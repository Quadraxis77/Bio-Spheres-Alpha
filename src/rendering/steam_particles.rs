//! Steam particle system renderer (GPU-based)
//!
//! Renders steam (fluid type 3) as billboarded quads using GPU compute
//! to extract steam positions directly from the fluid state buffer.

use bytemuck::{Pod, Zeroable};
use glam::Vec3;

/// Steam particle instance data (must match shader struct)
#[repr(C)]
...
/// Parameters for steam extraction compute shader
#[repr(C)]
#[derive(Copy, Clone, Debug, Pod, Zeroable)]
...
/// GPU-based steam particle system renderer
pub struct SteamParticleRenderer {
    // Render pipeline
    render_pipeline: wgpu::RenderPipeline,

    // Compute pipeline for extracting steam positions
...
impl SteamParticleRenderer {
    pub fn new(
        ...
        let render_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Steam Particle Render Shader"),
            source: wgpu::ShaderSource::Wgsl(
                include_str!("../../shaders/steam_particles.wgsl").into(),
            ),
        });

        // Create compute shader for extraction
        let extract_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Steam Extract Compute Shader"),
            source: wgpu::ShaderSource::Wgsl(
        ...
        let particle_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Steam Particle Buffer"),
            size: (std::mem::size_of::<SteamParticle>() * max_particles as usize) as u64,
        ...
        let counter_staging_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Steam Counter Staging"),
            size: std::mem::size_of::<ParticleCounter>() as u64,
        ...
        let params_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Steam Extract Params"),
        ...
        let vertex_buffer_layout = wgpu::VertexBufferLayout {
            array_stride: std::mem::size_of::<SteamParticle>() as u64,
        ...
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Steam Particle Pipeline"),
    ...
    pub fn create_render_bind_group(&self, device: &wgpu::Device) -> wgpu::BindGroup {
        device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("Steam Render Bind Group"),
            layout: &self.render_bind_group_layout,
            entries: &[],
        })
    }

    /// Run compute shader to extract steam particles from fluid state
