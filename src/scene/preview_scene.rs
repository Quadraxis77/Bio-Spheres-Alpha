...
pub struct PreviewScene {
    ...
    /// Accumulated scrubbed seconds since the last audible time tick.
    scrub_tick_remainder: f32,
    /// Audio events generated by preview-only interactions.
...
impl PreviewScene {
    ...
    pub fn sync_time_from_ui(&mut self, ui_time_value: f32, _max_duration: f32, is_dragging: bool) {
        ...
        if (ui_time_value - self.last_ui_time_value).abs() > 0.01 {
            ...
            if needs_update {
                self.queue_scrub_ticks((target_sim_time - current_target).abs(), is_dragging);
    ...
    fn queue_scrub_ticks(&mut self, scrubbed_seconds: f32, is_dragging: bool) {
        if scrubbed_seconds <= 0.0 {
            return;
        }

        if !is_dragging {
            self.scrub_tick_remainder = 0.0;
        }

        self.scrub_tick_remainder += scrubbed_seconds;
        let tick_count = (self.scrub_tick_remainder / SCRUB_TICK_INTERVAL_SECONDS).floor() as usize;
        if tick_count == 0 {
            return;
        }

        self.scrub_tick_remainder %= SCRUB_TICK_INTERVAL_SECONDS;
        for _ in 0..tick_count.min(MAX_SCRUB_TICKS_PER_FRAME) {
            self.pending_audio_events
                .push(crate::audio::GameAudioEvent::SliderTick);
        }
    }
}
